...
internal fun actionFlow(userSession: UserSessionScope, terminal: Terminal): Flow<InputAction> {
    ...
    return merge(
        ...
        .mapNotNull { event ->
            if (event.isCtrlC) {
                return@mapNotNull InputAction.Quit
            }
            ...
            when (event.key) {
                ...
                "Backspace", "Delete" -> {
                    ...
                    draftBuffer.insert(cursorPosition, event.key)
                    val message = draftBuffer.toString()
