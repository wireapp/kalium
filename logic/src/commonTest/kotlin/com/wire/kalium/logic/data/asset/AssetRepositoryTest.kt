...
class AssetRepositoryTest {
    ...
    @BeforeTest
    fun setUp() {
        assetRepository = AssetDataSource(assetApi)
    }
    ...
    fun givenValidParams_whenUploadingPublicAssets_thenShouldSucceedWithAMappedResponse() = runTest {
        given(assetApi)
            ...
            .thenReturn(NetworkResponse.Success(FakeHttpResponse(), Asset("some_key", "some_expiration_val", "some_token")))

        val uploadAssetMetadata = UploadAssetMetadata("image/jpg", true, RetentionType.ETERNAL)

        val actual = assetRepository.uploadPublicAsset(uploadAssetMetadata, "the_image".encodeToByteArray())
