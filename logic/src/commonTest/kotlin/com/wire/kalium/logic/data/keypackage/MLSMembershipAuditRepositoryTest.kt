...
class MLSMembershipAuditRepositoryTest {
    ...
    @Test
    fun givenPersistedMarker_whenObserving_thenAuditStateIsRequired() = runTest {
        val metadataDAO = mock<MetadataDAO>()
        every { metadataDAO.valueByKeyFlow(any()) } returns flowOf(null, "required")
    ...
    fun whenMarkingAndClearingAuditRequirement_thenMetadataIsPersistedAndDeleted() = runTest {
        val metadataDAO = mock<MetadataDAO>()
        everySuspend { metadataDAO.valueByKey(any()) } returns null
        ...
        verifySuspend(VerifyMode.exactly(1)) {
            metadataDAO.insertValue(eq("required"), any())
        }
        verifySuspend(VerifyMode.exactly(1)) {
    ...
    @Test
    fun givenDeferredState_whenMarkingAuditRequired_thenDeferralIsNotDowngraded() = runTest {
        val metadataDAO = mock<MetadataDAO>()
        everySuspend { metadataDAO.valueByKey(any()) } returns "required_after_slow_sync"
        everySuspend { metadataDAO.insertValue(any(), any()) } returns Unit
        val repository = MLSMembershipAuditRepositoryImpl(metadataDAO)
