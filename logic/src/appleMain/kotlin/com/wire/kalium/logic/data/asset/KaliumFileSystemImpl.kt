...
internal actual class KaliumFileSystemImpl actual constructor(
     ...
     */
    actual override val rootCachePath: Path = dataStoragePaths.cachePath.value.toPath()

    /**
     * Provides the root of the current user database path, used to store all the Database information.
     */
    actual override val rootDBPath: Path = dataStoragePaths.dbPath.value.toPath()
     ...
     */
    actual override fun sink(outputPath: Path, mustCreate: Boolean): Sink = SYSTEM.sink(outputPath, mustCreate)
     ...
     */
    actual override fun source(inputPath: Path): Source = SYSTEM.source(inputPath)
     ...
     */
    actual override fun createDirectories(dir: Path) = SYSTEM.createDirectories(dir, false)
     ...
     */
    actual override fun createDirectory(dir: Path, mustCreate: Boolean) = SYSTEM.createDirectory(dir, mustCreate)
     ...
     */
    actual override fun delete(path: Path, mustExist: Boolean) = SYSTEM.delete(path, mustExist)
     ...
     */
    actual override fun deleteContents(dir: Path, mustExist: Boolean) = SYSTEM.deleteRecursively(dir, mustExist)
     ...
     */
    actual override fun exists(path: Path): Boolean = SYSTEM.exists(path)
     ...
     */
    actual override fun copy(sourcePath: Path, targetPath: Path) = SYSTEM.copy(sourcePath, targetPath)
     ...
     */
    actual override fun tempFilePath(pathString: String?): Path {
     ...
     */
    actual override suspend fun readByteArray(inputPath: Path): ByteArray = withContext(dispatcher.io) {
     ...
     */
    actual override suspend fun writeData(outputSink: Sink, dataSource: Source): Long {
        var byteCount = 0L
     ...
     */
    actual override fun selfUserAvatarPath(): Path = providePersistentAssetPath("self_user_avatar.jpg")
     ...
     */
    actual override suspend fun listDirectories(dir: Path): List<Path> = SYSTEM.list(dir)

    actual override fun size(path: Path): Long = SYSTEM.metadata(path).size ?: 0L
}
