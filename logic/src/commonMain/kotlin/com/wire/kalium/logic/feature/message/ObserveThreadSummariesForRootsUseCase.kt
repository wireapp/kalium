...
public sealed interface ObserveThreadSummariesForRootsResult {
    public data class Success(val summaries: List<ThreadRootSummary>) : ObserveThreadSummariesForRootsResult
...
public class ObserveThreadSummariesForRootsUseCase internal constructor(
    ...
    ): Flow<ObserveThreadSummariesForRootsResult> =
        messageThreadRepository.observeThreadSummariesForRoots(conversationId, rootMessageIds).map { result ->
            result.fold(
                { ObserveThreadSummariesForRootsResult.Failure },
                { summaries ->
                    ObserveThreadSummariesForRootsResult.Success(
