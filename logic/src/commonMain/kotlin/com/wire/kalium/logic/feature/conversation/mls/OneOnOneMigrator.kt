...
internal class OneOnOneMigratorImpl(
    ...
    override suspend fun migrateToProteus(user: OtherUser): Either<CoreFailure, ConversationId> =
        conversationRepository.getOneOnOneConversationWithOtherUser(user.id, Conversation.Protocol.PROTEUS).flatMap { conversationId ->
            userRepository.updateOneOnOneConversation(user.id, conversationId)
    ...
    override suspend fun migrateToMLS(user: OtherUser): Either<CoreFailure, ConversationId> {
        return getResolvedMLSOneOnOne(user.id)
            .flatMap { mlsConversation ->
                if (user.oneOnOneConversationId == mlsConversation) {
    ...
    private suspend fun migrateOneOnOneHistory(user: OtherUser, targetConversation: ConversationId): Either<CoreFailure, Unit> {
        return if (user.userType.isTeammate()) {
            val teamId = teamIdProvider().getOrNull() ?: return Either.Left(StorageFailure.DataNotFound)
        ...
        } else if (user.oneOnOneConversationId != null) {
            messageRepository.moveMessagesToAnotherConversation(
                originalConversation = user.oneOnOneConversationId,
                targetConversation = targetConversation
            )
        } else {
            Either.Right(Unit)
