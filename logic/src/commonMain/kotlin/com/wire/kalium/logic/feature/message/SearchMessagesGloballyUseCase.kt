...
/**
 * Use case for searching messages across all conversations by text content.
 */
public interface SearchMessagesGloballyUseCase {
...
internal class SearchMessagesGloballyUseCaseImpl internal constructor(
    ...
    override suspend fun invoke(
        ...
        limit: Int,
        offset: Int
    ): SearchMessagesGloballyUseCase.Result = withContext(dispatcher.io) {
        messageRepository.searchMessagesByTextGlobally(
            searchQuery = searchQuery,
            limit = limit,
            offset = offset
        ).fold(
            { SearchMessagesGloballyUseCase.Result.Failure(it) },
            { SearchMessagesGloballyUseCase.Result.Success(it) }
        )
    }
}
