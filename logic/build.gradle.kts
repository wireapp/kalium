import Customization.buildtimeConfiguration
...
buildkonfig {
    ...
    defaultConfigs {
        ...
        buildConfigField(
            BOOLEAN,
            Configs.allow_account_creation.name, "${buildtimeConfiguration?.configuration?.get(Configs.allow_account_creation.name)}"
        )
        buildConfigField(
            BOOLEAN,
        ...
        buildConfigField(
            STRING,
            Configs.firebasePushSenderId.name, "${buildtimeConfiguration?.configuration?.get(Configs.firebasePushSenderId.name)}"
        )
        buildConfigField(
            STRING,
            Configs.firebaseAppId.name, "${buildtimeConfiguration?.configuration?.get(Configs.firebaseAppId.name)}"
        )

        buildConfigField(
            BOOLEAN,
            Configs.submitCrashReports.name, "${buildtimeConfiguration?.configuration?.get(Configs.submitCrashReports.name)}"
        )
        ...
        buildConfigField(
            BOOLEAN,
            Configs.enableBlacklist.name, "${buildtimeConfiguration?.configuration?.get(Configs.enableBlacklist.name)}"
        )

        buildConfigField(
            STRING,
            Configs.blacklistHost.name, "${buildtimeConfiguration?.configuration?.get(Configs.blacklistHost.name)}"
        )

        buildConfigField(
            BOOLEAN,
            Configs.allowChangeOfEmail.name, "${buildtimeConfiguration?.configuration?.get(Configs.allowChangeOfEmail.name)}"
        )
        ...
        buildConfigField(
            STRING,
            Configs.custom_url_scheme.name, "${buildtimeConfiguration?.configuration?.get(Configs.custom_url_scheme.name)}"
        )
        ...
        buildConfigField(
            STRING,
            Configs.http_proxy_url.name, "${buildtimeConfiguration?.configuration?.get(Configs.http_proxy_url.name)}"
        )

        buildConfigField(
            STRING,
            Configs.http_proxy_port.name, "${buildtimeConfiguration?.configuration?.get(Configs.http_proxy_port.name)}"
        )
        ...
        buildConfigField(
            BOOLEAN,
            Configs.force_hide_screen_content.name, "${buildtimeConfiguration?.configuration?.get(Configs.force_hide_screen_content.name)}"
        )

        buildConfigField(
            BOOLEAN,
            Configs.force_app_lock.name, "${buildtimeConfiguration?.configuration?.get(Configs.force_app_lock.name)}"
        )

        buildConfigField(
            INT,
            Configs.app_lock_timeout.name, "${buildtimeConfiguration?.configuration?.get(Configs.app_lock_timeout.name)}"
        )

        buildConfigField(
            BOOLEAN,
            Configs.block_on_password_policy.name, "${buildtimeConfiguration?.configuration?.get(Configs.block_on_password_policy.name)}"
        )

        buildConfigField(
            BOOLEAN,
            Configs.wipe_on_cookie_invalid.name, "${buildtimeConfiguration?.configuration?.get(Configs.wipe_on_cookie_invalid.name)}"
        )

        buildConfigField(
            BOOLEAN,
            Configs.force_private_keyboard.name, "${buildtimeConfiguration?.configuration?.get(Configs.force_private_keyboard.name)}"
        )

        buildConfigField(
            INT,
            Configs.password_max_attempts.name, "${buildtimeConfiguration?.configuration?.get(Configs.password_max_attempts.name)}"
        )

        buildConfigField(
            BOOLEAN,
            Configs.file_restriction_enabled.name, "${buildtimeConfiguration?.configuration?.get(Configs.file_restriction_enabled.name)}"
        )

        buildConfigField(
            STRING,
            Configs.file_restriction_list.name, "${buildtimeConfiguration?.configuration?.get(Configs.file_restriction_list.name)}"
        )

        buildConfigField(
            STRING,
            Configs.countly_server_url.name, "${buildtimeConfiguration?.configuration?.get(Configs.countly_server_url.name)}"
        )

        buildConfigField(
            STRING,
            Configs.countly_app_key.name, "${buildtimeConfiguration?.configuration?.get(Configs.countly_app_key.name)}"
        )
        ...
        buildConfigField(
            BOOLEAN,
            Configs.web_link_preview.name, "${buildtimeConfiguration?.configuration?.get(Configs.web_link_preview.name)}"
        )

        buildConfigField(
            BOOLEAN,
            Configs.keep_websocket_on.name, "${buildtimeConfiguration?.configuration?.get(Configs.keep_websocket_on.name)}"
        )

        val certificatePinMap = buildtimeConfiguration?.configuration?.get(Configs.certificatePin.name) as Map<String, String>
        buildConfigField(
            STRING,
            "${Configs.certificatePin.name}_${Configs.domain.name}", "${certificatePinMap.get(Configs.domain.name)}"
        )
