...
import com.wire.backup.file.header.BackupHeader
import com.wire.backup.file.header.BackupHeader.HeaderDecodingErrors
import com.wire.backup.file.header.BackupHeader.HeaderDecodingErrors.INVALID_FORMAT
import com.wire.backup.file.header.BackupHeader.HeaderDecodingErrors.INVALID_USER_ID
import com.wire.backup.file.header.BackupHeader.HeaderDecodingErrors.INVALID_VERSION
import okio.Buffer
...
internal class BackupCoder(val userId: BackupQualifiedId, val passphrase: BackupPassphrase) {
    ...
    fun decodeHeader(encryptedDataSource: Source): Pair<HeaderDecodingErrors?, BackupHeader> {
        ...
        // Sanity checks
        val expectedHashedUserId = hashUserId(userId, decodedHeader.encryptionSalt, decodedHeader.operationsLimit, decodedHeader.hashingMemoryLimit)
...
//                 kaliumLogger.e("The backup version found in the backup file header is not a valid one")
    ...
    private fun Source.readBackupHeader(): BackupHeader {
        ...
        val opslimit = this.read(readBuffer, UNSIGNED_INT_LENGTH)
