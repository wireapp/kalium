...
public fun interface UserUpdateEventHandler {
    public suspend fun handle(event: Event.User.Update): Either<CoreFailure, Unit>
}
...
public class UserUpdateEventHandlerImpl public constructor(
    ...
    override suspend fun handle(event: Event.User.Update): Either<CoreFailure, Unit> {
        ...
        return userRepository.updateUserFromEvent(event)
            ...
            .flatMapLeft {
                if (it is StorageFailure.DataNotFound) Either.Right(Unit) else Either.Left(it)
            }
    }
}
