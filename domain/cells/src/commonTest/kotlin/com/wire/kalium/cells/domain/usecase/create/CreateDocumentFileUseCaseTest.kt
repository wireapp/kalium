...
class CreateDocumentFileUseCaseTest {
    ...
    fun `given path, when invoke is called, then it calls repository with correct parameters`() = runTest {
        ...
        coVerify {
            arrangement.cellsRepository.createFile(
                folderName = path + CreateDocumentFileUseCaseImpl.EXTENSION,
                contentType = CreateDocumentFileUseCaseImpl.CONTENT_TYPE,
                templateUuid = CreateDocumentFileUseCaseImpl.DOC_TEMPLATE_UUID
            )
        }.wasInvoked()
    }
    ...
    private class Arrangement {
        ...
        fun arrange() = this to CreateDocumentFileUseCaseImpl(
