...
import com.wire.kalium.persistence.db.Api28Impl.setWindowSize
...
class AndroidSqliteDriver private constructor(
    ...
    private fun <T> execute(
        ...
        try {
            ...
            val queryPlanLogger = kaliumLogger.withTextTag("SQL_QUERY_PLAN")
            try {
                if (statementString.startsWith("SELECT")) {
                    val planResult = DatabaseUtils.dumpCursorToString(
                        database.query("EXPLAIN QUERY PLAN $statementString")
                    )
                    if (planResult.contains("detail=SCAN")) {
                        queryPlanLogger.e("Plan for query contains scans: '$statementString': $planResult")
                    } else {
                        queryPlanLogger.i("Plan for query: '$statementString': $planResult")
                    }
                    queryPlanLogger
                }
            } catch (e: Throwable) {
                queryPlanLogger.e("FAILURE TO GET PLAN for query: '$statementString'", e)
            }
