...
object NotificationEventsResponseJson {
    ...
    private val newSelfDeletingMessagesFeatureConfigUpdate = ValidJsonProvider(
        serializableData = EventContentDTO.FeatureConfig.FeatureConfigUpdatedDTO(
            SelfDeletingMessages(
                config = SelfDeletingMessagesConfigDTO(enforcedTimeoutSeconds = 60),
                status = FeatureFlagStatusDTO.ENABLED,
            )
        ),
        jsonProvider = newFeatureConfigSerializer
    )

    private val newAppLockFeatureConfigUpdate = ValidJsonProvider(
        EventContentDTO.FeatureConfig.FeatureConfigUpdatedDTO(
            AppLock(
                config = AppLockConfigDTO(
                    enforceAppLock = true,
    ...
    val mostRecentEvent = EventResponse(
        id = "eventId",
        payload = listOf(),
        transient = false
    )
