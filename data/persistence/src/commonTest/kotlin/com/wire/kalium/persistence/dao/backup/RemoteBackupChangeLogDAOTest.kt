...
class RemoteBackupChangeLogDAOTest : BaseDatabaseTest() {
    ...
    fun givenNoEntries_whenLoggingMessageUpsert_thenEntryIsStored() = runTest(dispatcher) {
        ...
        with(result.first()) {
            ...
            assertEquals(MESSAGE_NONCE_1, messageNonce)
    ...
    @Test
    fun givenNoEntries_whenObservingPendingChanges_thenEmptyListIsEmitted() = runTest(dispatcher) {
        val result = dao.observePendingChanges().first()

        assertTrue(result.isEmpty())
    }

    @Test
    fun givenEntries_whenObservingPendingChanges_thenEntriesAreOrderedByTimestamp() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_3)
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_2, TIMESTAMP_1)
        dao.logMessageDelete(CONVERSATION_ID_2, MESSAGE_NONCE_1, TIMESTAMP_2)
    ...
    @Test
    fun givenEntries_whenNewEntryIsAdded_thenObservableEmitsUpdatedList() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)

        dao.observePendingChanges().test {
    ...
    @Test
    fun givenNoEntries_whenGettingByEventType_thenEmptyListIsReturned() = runTest(dispatcher) {
        val result = dao.getChangesByEventType(ChangeLogEventType.MESSAGE_UPSERT)

        assertTrue(result.isEmpty())
    }

    @Test
    fun givenMixedEntries_whenGettingByEventType_thenOnlyMatchingEntriesAreReturned() = runTest(dispatcher) {
        val conversationId3 = QualifiedIDEntity("conv3", "domain.com")

        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)
        dao.logMessageUpsert(CONVERSATION_ID_2, MESSAGE_NONCE_2, TIMESTAMP_2)
        dao.logMessageDelete(CONVERSATION_ID_1, MESSAGE_NONCE_2, TIMESTAMP_1)
        dao.logReactionsSync(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)
        dao.logConversationClear(conversationId3, TIMESTAMP_1)

        val upsertResults = dao.getChangesByEventType(ChangeLogEventType.MESSAGE_UPSERT)
        val deleteResults = dao.getChangesByEventType(ChangeLogEventType.MESSAGE_DELETE)
        val reactionResults = dao.getChangesByEventType(ChangeLogEventType.REACTIONS_SYNC)
        val clearResults = dao.getChangesByEventType(ChangeLogEventType.CONVERSATION_CLEAR)

        assertEquals(2, upsertResults.size)
        assertTrue(upsertResults.all { it.eventType == ChangeLogEventType.MESSAGE_UPSERT })

        assertEquals(1, deleteResults.size)
        assertEquals(ChangeLogEventType.MESSAGE_DELETE, deleteResults.first().eventType)

        assertEquals(1, reactionResults.size)
        assertEquals(ChangeLogEventType.REACTIONS_SYNC, reactionResults.first().eventType)

        assertEquals(1, clearResults.size)
        assertEquals(ChangeLogEventType.CONVERSATION_CLEAR, clearResults.first().eventType)
    }

    @Test
    fun givenEntriesWithSameEventType_whenGettingByEventType_thenResultsAreOrderedByTimestamp() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_3)
        dao.logMessageUpsert(CONVERSATION_ID_2, MESSAGE_NONCE_2, TIMESTAMP_1)
    ...
    @Test
    fun givenNoEntries_whenGettingForConversation_thenEmptyListIsReturned() = runTest(dispatcher) {
        val result = dao.getChangesForConversation(CONVERSATION_ID_1)

        assertTrue(result.isEmpty())
    }

    @Test
    fun givenEntriesInMultipleConversations_whenGettingForConversation_thenOnlyMatchingEntriesAreReturned() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)
        dao.logMessageDelete(CONVERSATION_ID_1, MESSAGE_NONCE_2, TIMESTAMP_2)
        dao.logMessageUpsert(CONVERSATION_ID_2, MESSAGE_NONCE_1, TIMESTAMP_1)
        dao.logReactionsSync(CONVERSATION_ID_2, MESSAGE_NONCE_2, TIMESTAMP_2)

        val conversation1Results = dao.getChangesForConversation(CONVERSATION_ID_1)
        val conversation2Results = dao.getChangesForConversation(CONVERSATION_ID_2)

        assertEquals(2, conversation1Results.size)
        assertTrue(conversation1Results.all { it.conversationId == CONVERSATION_ID_1 })

        assertEquals(2, conversation2Results.size)
        assertTrue(conversation2Results.all { it.conversationId == CONVERSATION_ID_2 })
    }

    @Test
    fun givenEntriesInConversation_whenGettingForConversation_thenResultsAreOrderedByTimestamp() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_3)
        dao.logMessageDelete(CONVERSATION_ID_1, MESSAGE_NONCE_2, TIMESTAMP_1)
        dao.logReactionsSync(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_2)
    ...
    @Test
    fun givenNoEntries_whenDeletingProcessedEntries_thenNoErrorOccurs() = runTest(dispatcher) {
        dao.deleteProcessedEntries(TIMESTAMP_1)
    ...
    @Test
    fun givenEntries_whenDeletingProcessedEntriesUpToTimestamp_thenOnlyMatchingEntriesAreDeleted() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_2, TIMESTAMP_2)
        dao.logMessageUpsert(CONVERSATION_ID_2, MESSAGE_NONCE_1, TIMESTAMP_3)

        dao.deleteProcessedEntries(TIMESTAMP_2)

        val result = dao.getPendingChanges()
        assertEquals(1, result.size)
        assertEquals(TIMESTAMP_3, result.first().timestampMs)
    }

    @Test
    fun givenEntries_whenDeletingWithExactTimestamp_thenEntriesWithThatTimestampAreDeleted() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_2, TIMESTAMP_1)
    ...
    @Test
    fun givenNoEntries_whenDeletingSpecificEntry_thenNoErrorOccurs() = runTest(dispatcher) {
        dao.deleteEntry(CONVERSATION_ID_1, MESSAGE_NONCE_1, ChangeLogEventType.MESSAGE_UPSERT)
    ...
    @Test
    fun givenEntry_whenDeletingByCompositeKey_thenEntryIsDeleted() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)
    ...
    @Test
    fun givenMultipleEventTypesForSameMessage_whenDeletingOneEventType_thenOtherEventTypesRemain() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)
        dao.logMessageDelete(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_2)
        ...
        val result = dao.getPendingChanges()
        assertEquals(2, result.size)
        assertTrue(result.none { it.eventType == ChangeLogEventType.MESSAGE_UPSERT })
    ...
    @Test
    fun givenEntriesInMultipleConversations_whenDeletingFromOne_thenOtherConversationsAreUnaffected() = runTest(dispatcher) {
        dao.logMessageUpsert(CONVERSATION_ID_1, MESSAGE_NONCE_1, TIMESTAMP_1)
        dao.logMessageUpsert(CONVERSATION_ID_2, MESSAGE_NONCE_1, TIMESTAMP_2)
        ...
        val result = dao.getPendingChanges()
        assertEquals(1, result.size)
        assertEquals(CONVERSATION_ID_2, result.first().conversationId)
    }
    ...
    fun givenDifferentMessagesInSameConversation_whenLogging_thenAllEntriesAreStored() = runTest(dispatcher) {
        ...
        assertEquals(2, result.size)
        assertTrue(result.any { it.messageNonce == MESSAGE_NONCE_1 })
        assertTrue(result.any { it.messageNonce == MESSAGE_NONCE_2 })
    }
